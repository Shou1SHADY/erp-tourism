...
// Stub client if missing credentials to prevent crash on startup
...
export async function getClientToken() {
  if (!oAuthAuthorizationController) return "mock_token";
...
export async function createPaypalOrder(req: Request, res: Response) {
  try {
    if (!ordersController) return res.status(500).json({ error: "PayPal not configured" });
